...
import pytest
import pandas as pd
from stonks.scraper import download_historical_data, run_stocks_analysis, calculate_indicators
...
class TestScraper:
    ...
    def test_given_empty_dataframe_is_returned_when_passing_dataframe_then_exception_is_being_thrown(self):
        # given
        # when
        with patch('stonks.scraper.download_historical_data', return_value=pd.DataFrame()):
            # then
            with pytest.raises(ValueError):
                run_stocks_analysis('INVALID_ASSET')

    def test_given_incorrect_asset_name_when_downloading_historical_data_then_empty_dataframe_is_returned(self):
        # given
        asset = 'NOT_STONKS'
        # when
        df = download_historical_data(asset)
        # then
